...
int getSignalResult(double metCut, double jetptCut,  double ptCut, double ecaloCut, double iasCut, TString region){
  ...
  for(unsigned int i=0; i<samples.size(); i++){
    ...
    filename = "results/Signal_" + samples[i].filename  + Form("_metCutEq%.0f_ptCutEq%.0f_ECaloCutEq%.0f_IasCutEq0p%02.0f.root",metCut,ptCut,ecaloCut,iasCut*100);
    ...
    samples[i].Selection(  0, 0, metCut, jetptCut, ptCut, iasCut, 1,          ecaloCut, 0, region);
    double nSRError  = 0;
