...
int calculateSignificance(double metCut, double ecaloCut, TString region, TString folder = "NLostOuterGe0"){
  ...
  for(unsigned int i=0; i<samples.size(); i++){
	...
	for(int k=0; k<1000; k++){
	  ...
	  sOverDeltabUpStatPlusSys            = k/sqrt( pow( getOneSidedUpperLimit(nAll,0.6827)-nAll ,2) + pow(fakeStatError ,2) + pow(leptonStatError ,2) + pow(fakeSysError ,2) + pow(leptonSysError ,2) +pow(0.2,2));
