...
//################################################################################################################################
//################################################################################################################################
int a3_getFakePrediction(double metCut, double jetptCut, double ecaloCut=5., double iasCut=0.3, double ptCut=35.){
  ...
  int bin          = fakeRates->GetXaxis()->FindBin(Form("%.0f_%.0f",ptCut,ecaloCut));
  ...
  //%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Fake Prediction in CR1+CR2 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  filename = Form("templates/ECaloTemplate_METGt%.0f_JetPtGt%.0f_trackPtGt%.0f.root",metCut,jetptCut,ptCut);
  ...
  filename             =   Form("templates/IasTemplate_METGt%.0f_JetPtGt%.0f_trackPtGt%0.f_ECaloEq%.0f.root",metCut,jetptCut,ptCut,ecaloCut);
  cout<<"Ias template from :"<<filename<<endl;
  TFile *inIas         =   new TFile(filename,"READ");
  TH1D* histoLowECalo  = 0;
  TH1D* histoHighECalo = 0;
  histoName            = Form("fake_bkg_data_Iascut_0p%02.0f_EcaloLow_1",iasCut*100);
  cout<<"histogram low ECalo = "<<histoName<<endl; 
  inIas  ->  GetObject(histoName,histoLowECalo);
  histoName            = Form("fake_bkg_data_Iascut_0p%02.0f_EcaloLow_0",iasCut*100);
  ...
  //%%%%%%%%%%%%%%%%%%%%  Write output %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  filename = Form("results/fakeBkg_metCutEq%.0f_ptCutEq%.0f_ECaloCutEq%.0f_IasCutEq0p%02.0f.root",metCut,ptCut,ecaloCut,iasCut*100);
