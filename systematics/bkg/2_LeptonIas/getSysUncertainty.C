...
public:
  ...
  void Selection(int pdgId, bool doKinSelection=false, bool doMatching=false, double ptCut=20, double ecaloCut=5, double iasCut=0.1){

    statistics = 0;
    purity=0;
    double auxWeight = 0;

    double bins[3] = {0,iasCut,1};
    histo= new TH1D("hASmi","hASmi",2,bins);
    histo->Sumw2();
...
int getSysUncertainty(int pdgId, double ptCut, double ecaloCut, double iasCut){
  ...
  if(highIasDataCR==0){
    ...
    for(int i=1; i<10; i++){
      ...
      mcCR.Selection(  pdgId, 0, 0, ptCut, ecaloCut, iasCut - i*0.05);
      dataCR.Selection(pdgId, 0, 0, ptCut, ecaloCut, iasCut - i*0.05);
      if(dataCR.histo->GetBinContent(2)!=0) break;
    }
    
    highIasMCCR             = mcCR.histo->GetBinContent(2);
    highIasDataCR           = dataCR.histo->GetBinContent(2);
    scaling = dataCR.histo->Integral()/mcCR.histo->Integral();
    highIasMCCR          = highIasMCCR         * scaling;

    ratio        = highIasMCCR/highIasDataCR;
    ratioErrorUp   = ratio* sqrt(  pow( highIasMCCRErrorUp/highIasMCCR ,2 )   +  pow( highIasDataCRErrorLow/highIasDataCR ,2 ) );
    ratioErrorLow  = ratio* sqrt(  pow( highIasMCCRErrorLow/highIasMCCR ,2 )  +  pow( highIasDataCRErrorUp/highIasDataCR ,2 ) );
